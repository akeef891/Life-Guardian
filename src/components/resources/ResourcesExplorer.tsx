...
import { scheduleIdleTask } from "@/lib/performance/schedule-idle";
...
const LocationPlaceDetails = dynamic(
  ...
  {
    loading: () => (
      <p className="rounded-xl border border-border bg-background p-4 text-xs text-muted">
...
export function ResourcesExplorer() {
  ...
  const [shouldAcquireGps, setShouldAcquireGps] = useState(false);
  ...
  useEffect(() => {
    ...
    return cancel;
  }, [shouldAcquireGps, loadResources]);
  ...
  return (
    <div ref={panelRef} className="min-w-0 space-y-6 overflow-x-hidden">
      ...
      {error ? (
        ...
        >
          <p>{error}</p>
          {error === t.resources.locationError ? (
            <button
              type="button"
              onClick={() => void loadResources()}
              disabled={loading}
              className="mt-2 inline-flex min-h-10 items-center rounded-lg border border-red-300 bg-white px-3 py-2 text-xs font-semibold text-red-800 transition-colors hover:bg-red-50 focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-red-300/40 disabled:opacity-60"
            >
              {t.common.retry}
            </button>
          ) : null}
